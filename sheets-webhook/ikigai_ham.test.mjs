...
function fixture() {
  ...
  function addSheet(name, rows = []) {
    const sheet = {
      ...
      setName(next) { sheets.delete(name); name = next; sheets.set(name, sheet); },
...
test('campaign registration copies the original schema without sharing mutable arrays or widths', () => {
  ...
  assert.equal(s.SHEET_TAB_NAMES.Ikigai_Ham, '10월 이키 재능');
...
test('setup creates only target header row and is repeatable without data rows or notifications', () => {
  ...
  assert.equal(result.sheet, '10월 이키 재능');
  assert.equal(result.columns, 15);
  assert.deepEqual(sheets.get('10월 이키 재능').rows, [[...s.withCampaignMonthHeader(s.SHEET_SCHEMAS.Ikigai_Landing.headers)]]);
  assert.deepEqual(original.rows, [['existing'], ['preserved']]);
  assert.equal(calls.writes.filter(call => call.kind === 'append').length, 1);
  assert.equal(calls.writes.every(call => call.name === '10월 이키 재능'), true);
  assert.deepEqual(calls.notified, []);
  assert.deepEqual(calls.overview, []);
});

test('readiness reads only the exact target header; never appends, acquires a lock or notifies', () => {
  const { sandbox: s, calls, addSheet } = fixture();
  addSheet('10월 이키 재능', [[...s.withCampaignMonthHeader(s.SHEET_SCHEMAS.Ikigai_Landing.headers)], ['PRIVATE DATA']]);
  const response = s.doPost(request({ sheetTab: '10월 이키 재능', action: 'ikigai_ham_readiness' }));
  assert.deepEqual(response, { ok: true, ready: true, sheetId: 'Ikigai_Ham', sheet: '10월 이키 재능', schemaVersion: 'ikigai-ham-v1' });
  assert.deepEqual(calls.lookup, ['10월 이키 재능']);
  assert.deepEqual(calls.reads, [{ name: '10월 이키 재능', row: 1, rowCount: 1 }]);
...
test('readiness fails closed for absent target, missing headers and duplicate headers without repair', () => {
  for (const state of ['absent', 'missing', 'duplicate', 'alias-only']) {
    const { sandbox: s, calls, addSheet } = fixture();
    ...
    if (state !== 'absent') addSheet(state === 'alias-only' ? 'Ikigai_Ham' : '10월 이키 재능', [headers]);
...
test('both ham routing names preserve every base field and write only to the new tab', () => {
  for (const sheetTab of ['Ikigai_Ham', '10월 이키 재능']) {
    const { sandbox: s, calls, sheets, addSheet } = fixture();
    const original = addSheet('이키가이 기존', [['existing'], ['preserved']]);
    ...
    assert.equal(result.sheet, '10월 이키 재능');
    const [headers, row] = sheets.get('10월 이키 재능').rows;
    ...
    assert.match(message, /10월 이키 재능/);
...
test('original Ikigai routing stays isolated from the ham tab; unknown targets remain rejected', () => {
  const { sandbox: s, calls, sheets, addSheet } = fixture();
  const ham = addSheet('10월 이키 재능', [['existing'], ['preserved']]);
